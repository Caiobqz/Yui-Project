...
async def lifespan(_app: FastAPI):
    # Fail-fast: configuração, identidade, personalidade e provedores (LLM
    # principal, utilitário e embeddings) são validados no boot. Uma API key
    # ausente falha AQUI, não na primeira requisição.
    settings.validate_runtime()
    identity_prompt()
    get_personality()
    get_llm_provider()
    get_utility_llm_provider()
    # Validação assíncrona (ex.: conectividade com um servidor Ollama).
    # Providers cuja disponibilidade já é garantida no __init__ síncrono
    # (Claude/OpenAI: API key; llama.cpp: arquivo GGUF) usam o no-op
    # default — este é o único ponto que precisa saber que ISSO existe,
    # nunca qual backend está por trás.
